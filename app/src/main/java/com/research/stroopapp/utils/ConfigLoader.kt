package com.research.stroopapp.utils
...
import com.research.stroopapp.models.StroopConfig
import com.research.stroopapp.models.ValidationResult
...
/**
 * Utility class for loading and validating the research configuration from JSON.
 * Supports both external storage (for user modifications) and assets (fallback).
 * Handles all configuration file operations including loading, parsing, and validation.
 */
class ConfigLoader(private val context: Context) {
    ...
    private fun parseJsonConfig(jsonContent: String): StroopConfig? {
        return try {
            ...
            val tempGson = Gson()
            val jsonObject = tempGson.fromJson(jsonContent, com.google.gson.JsonObject::class.java)
                ?: return null
            ...
            requiredSections.forEach { section ->
                if (!jsonObject.has(section)) {
                    throw JsonSyntaxException("Missing required section: $section")
                }
            }

            // Parse the full configuration
            gson.fromJson(jsonContent, StroopConfig::class.java)
