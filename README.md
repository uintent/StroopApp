...
•	Utilizing voice recognition to record participant responses to the Stroop test
•	Measuring reaction times and error rates during the Stroop test
•	Transmitting collected Stroop test data to the Master app via local WiFi network
...
•	JSON message format for simple and readable data exchange
...
6.	Apps exchange JSON messages over established connection
7.	Automatic reconnection if connection drops
Data Transmission Strategy:
•	Real-time communication: TCP socket connection for commands and status
•	End-of-task data transfer: Complete Stroop results sent as single JSON message
•	Message format: Simple JSON objects for commands, responses, and data
•	Connection monitoring: Heartbeat messages to detect disconnections
Example JSON message structure:
{
  "messageType": "STROOP_RESULTS",
  "sessionId": "session_123",
  "taskId": "task_1",
  "data": {
    "totalStroops": 15,
    "correctResponses": 12,
    "averageReactionTime": 1250,
    "individualTimes": [1200, 1300, 1100, ...]
  }
}
...
FR-TM-005: The Master app shall allow the moderator to set the end condition for a task while it is active, choosing from: "Failed," "Success," or "Partial Success."
...
•	The task name
•	All data transferred from the Projector app for that task
•	All data recorded by the moderator for that task (end condition, ASQ scores if applicable)
...
FR-TM-009: The Master app shall provide an "Emergency Stop" button accessible at all times, allowing the moderator to cancel any ongoing process (e.g., active task, data entry).
FR-TM-010: When the "Emergency Stop" button is pressed, the Master app shall display a confirmation prompt with the following options for the current task's data:
...
FR-TM-021: The Master app shall provide a "Voice Recognition System Check" option on the start screen (after participant information has been provided and before task selection).
...
FR-ST-005: The Projector app shall utilize voice recognition (supporting multiple languages) to detect participant responses to the displayed Stroop word.
...
•	The Stroop stimuli shall only be displayed on the Projector app, not the Master app
•	Between tasks, the Projector app shall display a black screen
•	The initial countdown (3, 2, 1, 0) shall be displayed on a black background
•	Stroop color words shall be displayed on a white background
...
FR-UF-006: If the ASQ is not recorded due to the task ending with a condition other than "Success" (e.g., "Failed," "Partial Success," "Timed Out"), the Master app shall record "NA" for the ASQ data fields for that task in the JSON file.
...
o	Time required for task completion (from countdown end to task end)
o	End condition ("Failed," "Success," "Partial Success," "Timed Out")
...
NFR-PER-003: The system shall achieve high recognition accuracy (>95%) for configured color names in a quiet testing environment.
4.2. Usability & UI
...
NFR-REL-002: The system shall handle scenarios where voice recognition fails to detect a valid response or detects irrelevant speech, influencing the "incorrectly named" count.
NFR-REL-003: Data saving to JSON shall be robust, preventing data loss in unexpected application closures or errors.
NFR-REL-004: Handling of Non-Stroop Utterances If the voice recognition system detects speech that is not a valid color name from the configured list (FR-ST-006), it shall ignore this input, allowing the participant to continue attempting to name the correct color as long as the Stroop stimulus is displayed.
4.4. Compatibility
...
Phase 1 (MVP):
•	Basic local WiFi network connection with JSON message format
•	Simple NSD service registration and discovery with manual device selection
•	Core Stroop test functionality with basic voice recognition
•	Essential task management and data collection
Phase 2 (Enhanced):
•	Advanced voice recognition with cloud fallback and calibration features
...
7.3. Voice Recognition Implementation Architecture
FR-ARCH-009: Google Cloud API Authentication Use Service Account authentication with JSON key stored in app assets for predictable billing and offline-first operation without requiring user authentication.
FR-ARCH-010: Audio File Handling Strategy Use temporary in-memory storage (ByteArray) for audio samples to avoid file system storage permissions complexity and ensure privacy.
FR-ARCH-011: Recognition Engine Switching Logic Implement timeout-based handoff between Android and Cloud recognition with confidence threshold management.
...
Network Communication: Native Android NSD + TCP Sockets + JSON
Voice Recognition: Android SpeechRecognizer + Google Cloud Speech-to-Text
